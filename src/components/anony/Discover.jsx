      ...
      <div className="border-b border-gray-400 pb-7">
        ...
        <div className="my-2 flex items-center gap-3 flex-wrap">
          {discover.map((item, index) => (
            <button key={index} className="bg-gray-200 py-2 px-3 text-sm rounded-full">
              {item}
            </button>
          ))}
        </div>
        <button className="text-green-pastel-500 text-sm py-3 hover:text-black1">
          See more topics
        </button>
      </div>
      <div className="flex items-center flex-wrap gap-3 leading-3 pt-8">
        {discoverActions.map((action, index) => (
          <button key={index} className="text-md text-black1">
            {action}
          </button>
        ))}
      </div>
